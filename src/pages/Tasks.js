  ...
  async function save() {
    ...
    try {
      ...
      if (task?.id) {
        ...
        result = await supabase.from('tasks').insert({ ...sanitize(form), completed: false, user_id: user.id });
      }
...
export default function Tasks() {
  ...
  return (
    <div className="page">
      ...
      {done.length > 0 && (
        <>
          <div className="section-label" onClick={() => setShowCompleted(!showCompleted)} style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
            {showCompleted ? 'v' : '>'} Completed ({done.length})
          </div>
